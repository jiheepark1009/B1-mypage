...
const featuredProjects = [
  {
    ...
    description:
      "시즌 SNS 캠페인 게시물 데이터를 분석해 참여율이 높은 콘텐츠 유형과 발행 시간대를 도출했습니다.",
  ...
  },
  {
    ...
    description:
      "설문·구매 데이터를 바탕으로 고객 세그먼트를 나누고 세그먼트별 맞춤 마케팅 전략을 제안했습니다.",
    image: "images/thumb-marketing.svg",
    metrics: [
      { label: "세그먼트", value: "4개" },
  ...
  },
  {
    ...
    title: "EPL 득점 예측 모델",
    description:
      "기대득점(xG) 지표를 활용해 경기별 득점을 예측하는 머신러닝 모델을 만들고 성능을 비교했습니다.",
  ...
  },
  {
    ...
    description:
      "시즌 경기 데이터를 기반으로 팀별 승률 추이와 관중 수요를 예측하는 대시보드를 제작했습니다.",
    image: "images/thumb-data.svg",
    metrics: [
      { label: "예측 오차", value: "±8%" },
...
const loadRepos = async () => {
  renderSkeleton();
  setRepoStatus(
    '<i class="fa-solid fa-spinner fa-spin" aria-hidden="true"></i> GitHub 저장소를 불러오는 중입니다...',
  );
  ...
  try {
    ...
    if (repos.length === 0) {
      repoGrid.innerHTML = "";
      setRepoStatus(
        '<i class="fa-solid fa-box-open" aria-hidden="true"></i> 표시할 프로젝트가 없습니다.',
      );
      return;
    }
    ...
    setRepoStatus(
      `<i class="fa-solid fa-circle-check" aria-hidden="true"></i> 총 ${repos.length}개의 저장소를 불러왔습니다.`,
    );
  } catch (error) {
    repoGrid.innerHTML = "";
    const reason =
      error.message === "rate-limit" ? " (API 요청 한도를 초과했습니다)" : "";
    setRepoStatus(
      `
...
contactForm.addEventListener("submit", (event) => {
  ...
  const isFormValid = Object.keys(fieldRules).map(validateField).every(Boolean);
